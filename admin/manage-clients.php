 ...
 <!-- End Sidebar-->
  ...
  <main id="main" class="main">
    ...
    <div class="container-fluid p-2">
        <h1 class="text-primary mb-2">Manage Clients</h1>
...
if (isset($_POST["submit"])) {
    ...
    if (!empty($name) || !empty($file['name']) || !empty($desc)) {
        $sql3 = "CREATE TABLE IF NOT EXISTS client (
            ID int(50) AUTO_INCREMENT,
            client_name varchar(255) NOT NULL,
            file_content BLOB NOT NULL,  -- Changed to BLOB for base64 image data
            ...
            PRIMARY KEY (ID),
            CONSTRAINT UC_clientName UNIQUE (client_name),
        ...
        $checkIfExists = "SELECT * FROM Client WHERE client_name = '$name'";
        $result = $conn->query($checkIfExists);

        if ($result->num_rows > 0) {
            echo '<div id="alert" class="alert alert-warning" role="alert">
                Client name already taken. Please use a different name.
            </div>';
        } else {
            $uploadDir = '../uploads/';
            ...
            $newFileName = 'pptsdsds' . uniqid() . '.' . $fileExtension;
            ...
            if ($imgInfo !== false) {
                ...
                if ($imgWidth > 400 || $imgHeight > 175) {
                    $source = imagecreatefromstring(file_get_contents($file['tmp_name']));
                    $destination = imagecreatetruecolor(400, 175);
                    imagecopyresampled($destination, $source, 0, 0, 0, 0, 400, 175, $imgWidth, $imgHeight);
                    ...
                    // No need to resize, move the uploaded file to the server
                ...
                $sql4 = "INSERT INTO Client (client_name, file_content, file_path, original_name, description) 
          ...
          <div class="row align-content-center align-self-center justify-content-center">
            <div class="card col-lg-8 p-3">
              <div class="card-body">
                ...
                <form class="row g-3" action="manage-clients.php" enctype="multipart/form-data" method="post" name="submit" onsubmit="return validateForm(event)">

                  <?php
                  // Define variables for props
                  $input_name = 'client_name';
                  $input_placeholder = 'Enter client name';
                  ...
                  ?>
                  <span class="text-alerts text-danger" id="client_name_error"></span> <!-- Error span for service name -->
                  ...
                  $input_placeholder = 'Enter Description For Client Work Here';
                  $input_class = 'form-control'; // Add your desired class
...
if (isset($_POST['change_status'])) {
    ...
    $statusQuery = "SELECT status FROM Client WHERE ID = $id";
    ...
    if ($statusResult) {
        ...
        $updateQuery = "UPDATE Client SET status = $newStatus WHERE ID = $id";
...
if (isset($_POST['delete_selected'])) {
    if (!empty($_POST['delete_id'])) {
        ...
        $deleteQuery = "SELECT file_path FROM Client WHERE ID IN ($deleteIDs)";
        ...
        if ($result) {
            ...
            $deleteQuery = "DELETE FROM Client WHERE ID IN ($deleteIDs)";
...
?>
  ...
  <script src="assets/js/clientvalidation.js?v=<?php echo time(); ?>"></script>
</body>
